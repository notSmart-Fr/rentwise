...
const ChatBox = ({ contextType, contextId }) => {
  const { user } = useAuth();
  ...
  const fetchMessages = async () => {
    try {
      const data = await messageService.getMessages(contextType, contextId);
      setMessages(data);
      setLoading(false);
    } catch (error) {
  ...
  const handleSend = async (e) => {
    ...
    try {
      await messageService.sendMessage(contextType, contextId, optimisticMsg.content);

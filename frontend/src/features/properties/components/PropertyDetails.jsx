...
const PropertyDetails = () => {
  ...
  const { leases, loading: leasesLoading } = useLeases(false);
  const activeLease = leases?.find(l => String(l.property_id) === String(id) && l.status === 'ACTIVE');
      ...
      <main className="relative z-10 max-w-[1920px] mx-auto pt-32 px-6 lg:px-20 pb-32">
        ...
        <header className="mb-20 animate-in fade-in slide-in-from-bottom-8 duration-700">
          <nav className="mb-8 flex items-center gap-4 text-[10px] font-black uppercase tracking-[0.3em] text-on-surface-variant overflow-x-auto whitespace-nowrap scrollbar-hide">
            <Link to="/" className="hover:text-primary transition-colors shrink-0">Portfolio</Link>
            <span className="opacity-20 shrink-0">/</span>
            <span className="hover:text-primary transition-colors shrink-0">{property.city}</span>
            <span className="opacity-20 shrink-0">/</span>
            <span className="text-white shrink-0">Active Asset</span>
          </nav>

          <div className="max-w-5xl">
            <h1 className="text-4xl sm:text-6xl lg:text-8xl font-black tracking-tightest leading-[0.9] text-white mb-8">
              {property.title}
            </h1>

            <div className="flex flex-col md:flex-row md:items-center gap-8 md:gap-16">
              <div className="flex items-center gap-3 text-on-surface-variant text-base md:text-lg font-medium italic">
                <span className="material-symbols-outlined text-primary-rentwise">location_on</span>
              ...
              <div className="flex flex-wrap gap-8 md:gap-10">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-xl font-black tracking-tight text-white">4 <span className="text-[10px] text-on-surface-variant font-black uppercase tracking-widest">Guests</span></p>
                </div>
                <div className="flex items-center gap-3">
                  <span className="material-symbols-outlined text-secondary text-sm">bed</span>
                  <p className="text-xl font-black tracking-tight text-white">{property.bedrooms || 0} <span className="text-[10px] text-on-surface-variant font-black uppercase tracking-widest">Beds</span></p>
                </div>
                <div className="flex items-center gap-3">
                  <span className="material-symbols-outlined text-tertiary text-sm">bathtub</span>
                  <p className="text-xl font-black tracking-tight text-white">{property.bathrooms || 0} <span className="text-[10px] text-on-surface-variant font-black uppercase tracking-widest">Baths</span></p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 mb-20">
          <div className="md:col-span-8 aspect-16/10 rounded-3xl md:rounded-[3rem] overflow-hidden bg-surface-container-low group relative shadow-2xl">
            <img
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-20">
          ...
          <div className="lg:col-span-8 space-y-20">
            ...
            <section className="space-y-8">
              <h3 className="text-[10px] font-black tracking-[0.3em] uppercase text-primary">Portfolio Intelligence</h3>
              <div className="space-y-6 text-xl leading-relaxed text-on-surface-variant max-w-4xl font-medium">
                <p>{property.description || "A verified residential asset optimized for long-term residency and financial stability. This property is part of a professionally managed portfolio."}</p>
              </div>
            </section>

            {/* Amenities Grid */}
            <section className="space-y-12">
              <h3 className="text-[10px] font-black tracking-[0.3em] uppercase text-secondary">Property Provisions</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {[
                  { title: 'Secured Entry', desc: 'Managed access points with biometric verification.', icon: 'shield' },
                  { title: 'Climate Control', desc: 'Central HVAC systems optimized for efficiency.', icon: 'ac_unit' },
                  { title: 'Maintenance Priority', desc: 'Dedicated service channel for immediate residency support.', icon: 'verified' },
                  { title: 'Digital Ledger', desc: 'All payments and contracts managed through the RentWise cloud.', icon: 'account_balance' }
                ].map((item, idx) => (
          ...
          <aside className="lg:col-span-4 lg:sticky lg:top-32 h-fit space-y-8">
              ...
              <div className="relative z-10">
                {activeLease ? (
                  <div className="space-y-8">
                    <div>
                      <span className="text-primary text-[10px] font-black uppercase tracking-[0.3em] mb-4 block">Active Residency</span>
                      <h3 className="text-3xl font-black text-white leading-tight">Welcome Home, <br />Tenant</h3>
                    </div>

                    <div className="space-y-4">
                      <Link
                        to="/dashboard"
                        className="w-full bg-linear-to-br from-primary-rentwise to-primary-container text-[#0b1326] h-20 rounded-full font-black text-lg tracking-tight flex items-center justify-center gap-3 hover:shadow-[0_0_40px_rgba(192,193,255,0.3)] transition-all active:scale-95"
                      >
                  ...
                  <>
                    <div className="mb-12">
                      <span className="text-on-surface-variant text-[10px] font-black uppercase tracking-[0.3em] mb-4 block">Monthly Rent</span>
                      <div className="flex items-baseline gap-2">
                        <span className="text-sm font-bold text-primary-rentwise">৳</span>
                      ...
                      <div className={`mt-6 flex items-center gap-2 py-2 px-5 rounded-full w-fit ${property.is_available ? 'bg-secondary-container/20 text-secondary-rentwise' : 'bg-tertiary-container/20 text-tertiary-rentwise'}`}>
                        <span className="material-symbols-outlined text-[16px]">verified</span>
                        <span className="text-[9px] font-black tracking-[0.15em] uppercase">
                          {property.is_available ? 'Asset Available' : 'Currently Occupied'}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      {requestStatus === 'success' ? (
                        <div className="bg-secondary-container/10 p-8 rounded-4xl text-center space-y-4 border border-secondary-container/20">
                          <div className="w-16 h-16 bg-secondary-rentwise rounded-full flex items-center justify-center mx-auto text-surface-container-high">
                            <span className="material-symbols-outlined text-3xl">check</span>
                          </div>
                          <p className="font-black text-secondary-rentwise uppercase tracking-widest text-xs">Application Pending</p>
                        </div>
                      ) : (
                        <>
                          <button
                            ...
                            className="w-full bg-linear-to-br from-primary-rentwise to-primary-container text-[#0b1326] h-20 rounded-full font-black text-lg tracking-tight hover:shadow-[0_0_40px_rgba(192,193,255,0.3)] transition-all active:scale-95 disabled:opacity-50"
                          >
                            {requestStatus === 'loading' ? 'Processing...' : 'Apply for Residency'}
                          </button>
                          <button
                            onClick={handleOpenChat}
                            className="w-full bg-surface-container-highest text-white h-20 rounded-full font-bold text-lg hover:bg-surface-bright transition-colors flex items-center justify-center gap-3 active:scale-95"
            ...
            <div className="bg-surface-container-low rounded-4xl p-6 flex items-center gap-5 group">
              ...
              <button className="w-12 h-12 rounded-2xl bg-surface-container-highest flex items-center justify-center text-primary-rentwise hover:bg-primary-rentwise hover:text-white transition-all">
